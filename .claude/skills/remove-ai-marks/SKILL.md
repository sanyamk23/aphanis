...
- If creating or processing files (PNG, SVG, PDF, DOCX, HTML, Markdown), execute `python3 -m watermark_remover.cli clean-file <file_path>` or invoke the `sanitize_file` MCP tool to strip all C2PA, EXIF, XMP, and XML comments before delivering the file to the user.
