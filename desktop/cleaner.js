// Cleaner logic for Electron desktop app
// Same algorithm as browser extension and web version.
...
const SWAPS = {
  ...
  'in today\'s digital landscape':'today,','in today\'s digital age':'today,','in today\'s fast-paced world':'today,',
  'in today\'s modern world':'today,','in the modern era':'today,','in the contemporary landscape':'today,',
  'in the current landscape':'today,','in the ever-evolving landscape':'today,','in recent years':'lately,',
  'in recent times':'recently,','at this point in time':'now','at the present time':'now',
  'prior to this':'before','prior to that':'before',
  'as a result':'so,','as a consequence':'so,',
  'has the ability to':'can','possesses the ability to':'can',
  'has the capability of':'can','have the capability of':'can','has the potential to':'could',
  'plays a crucial role in':'helps','plays a pivotal role in':'helps',
  'serves as a cornerstone of':'is part of','serves as a linchpin of':'is part of',
  'brings to bear':'adds','brings to the table':'adds',
  'in the realm of':'in the area of','in the domain of':'in the area of',
  'harnesses the power of':'uses','leverages the power of':'uses',
  'utilizes':'uses','executes the':'does the','executes this':'does this',
  'conducts a comprehensive analysis':'analyzes','conducts a thorough analysis':'analyzes',
  'iterates through':'goes through each',
  'computes the results of':'calculates','ensures that':'makes sure',
  'facilitates the process of':'helps',
  'optimizes for':'improves','reminiscent of':'like',
  'heralds a new era of':'starts a new era of','marks a significant shift in':'changes how',
  ...
  'in essence':'basically,','as such':'therefore,',
  'to that end':'for that purpose,','in the final analysis':'in the end,',
  'when all is said and done':'in the end,',
  'needless to say':'','all things being equal':'',
  'as a general rule':'',
  'indeed':'actually',
  'resulting in':'causing',
  'provides a comprehensive':'gives a','provides a thorough':'gives a',
  'provides a detailed':'gives a','provides a complete':'gives a','provides a robust':'gives a',
  'demonstrates the':'shows the','showcase the':'shows the','illustrates the':'shows the',
  'exemplifies the':'shows the','epitomizes the':'represents the',
  'sheds light on the':'explains the',
  'is a testament to':'proves','is a stark reminder of':'reminds us of',
  'is a vivid illustration of':'illustrates','is a prime example of':'exemplifies',
  'paves the way for':'enables',
  'ushering in a new era of':'starting a new era of',
  'safe guard against':'prevent',
  'preserving the integrity of':'keeping','ensuring the integrity of':'keeping','maintaining the integrity of':'keeping',
  'the bottom line is that':'in short,',
  'in the event that':'if','has a tendency to':'tends to',
  'in a position to':'able to','make a decision':'decide',
  'come to a conclusion':'conclude','provide assistance to':'help',
...
const FILLERS = {
  ...
  'in light of the fact that':'since','a majority of':'most',
  'has the capability of':'can','have the capability of':'can',
  'serves to improve':'improves','serves to enhance':'enhances',
  'plays a crucial role in':'helps','plays a pivotal role in':'helps',
  'it is worth noting that':'note that','it is essential to':'remember to',
  'it is important to note that':'note that',
  'not to mention':'and','it is safe to assume that':'',
  'it is worth mentioning that':'note that','all things considered':'overall',
  'in the final analysis':'in the end,','when all is said and done':'in the end,',
  'at the conclusion':'finally,','as a general rule':'',
  'the bottom line is that':'in short,'
};
...
const TRANSITIONS = {
  ...
  'nonetheless,':'still,','in conclusion,':'overall,','as a matter of fact,':'in fact,',
  'as a result,':'so,','as a consequence,':'so,',
  'to that end,':'for that purpose,','in light of this,':'given this,',
...
const CONTRACTIONS = {
  ...
  'could not':"couldn't",'is not':"isn't",'are not':"aren't",'was not':"wasn't",
  'were not':"weren't",'have not':"haven't",'has not':"hasn't",'had not':"hadn't",
  'it is':"it's",'that is':"that's",'there is':"there's",
  'what is':"what's",'here is':"here's",'we are':"we're",'you are':"you're",
  'they are':"they're",'we have':"we've",'you have':"you've",'they have':"they've",
  'we will':"we'll",'you will':"you'll",'they will':"they'll",
  'we would':"we'd",'you would':"you'd",'they would':"they'd",
  'who is':"who's",'when is':"when's",'where is':"where's",'how is':"how's",'why is':"why's"
};
...
function fullClean(input) {
  ...
  for (const [k,v] of Object.entries(SWAPS).sort((a,b)=>b[0].length-a[0].length)) r = r.replace(new RegExp('\\b'+escapeRegex(k)+'\\b','gi'), v);
  for (const [k,v] of Object.entries(FILLERS).sort((a,b)=>b[0].length-a[0].length)) r = r.replace(new RegExp('\\b'+escapeRegex(k)+'\\b','gi'), v);
  for (const [k,v] of Object.entries(TRANSITIONS).sort((a,b)=>b[0].length-a[0].length)) r = r.replace(new RegExp(escapeRegex(k),'gi'), v);
  for (const [k,v] of Object.entries(CONTRACTIONS).sort((a,b)=>b[0].length-a[0].length)) r = r.replace(new RegExp('\\b'+escapeRegex(k)+'\\b','gi'), v);
  r = r.replace(/[ \t]+/g, ' ').trim();
  return r;
}
